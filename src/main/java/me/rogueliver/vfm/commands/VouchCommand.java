...
public class VouchCommand implements CommandExecutor {
    ...
    public VouchCommand(VouchForMe plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage(ColorUtil.color(plugin.getConfigManager().getMessage("prefix") + "<red>Usage: /vouch <player> [reason]"));
            return true;
        }
        ...
        String message = plugin.getConfigManager().getMessage("vouch-sent")
                .replace("{player}", target.getName());
        player.sendMessage(ColorUtil.color(message));

        try {
            Sound sound = Sound.valueOf(plugin.getConfig().getString("sounds.vouch-sent"));
